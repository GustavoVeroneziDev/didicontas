...
if ($action === 'excluir') {
    ...
    if ($url) {
        ...
        if (strpos($url, UPLOAD_URL) === 0) {
            $filepath = UPLOAD_DIR . basename($url);
    ...
    foreach ($arquivos as $arq) {
        if (is_file($arq) && basename($arq) !== '.htaccess') {
            ...
            if (!in_array($url, $urls_conhecidas)) {
                $imagens[] = ['imagem_url' => $url, 'imagem_pos' => 'center center', 'total_produtos' => 0, 'produtos_nomes' => ''];
                $urls_conhecidas[] = $url;
            }
        }
    }
...
<head>
    ...
    <style>
        ...
        .img-card:hover { border-color:rgba(59,130,246,0.4); box-shadow:0 8px 24px rgba(0,0,0,0.4); }
        .img-card.selecionado { border-color:var(--accent); box-shadow:0 0 0 2px rgba(59,130,246,0.35); }
        .img-card.quebrada { border-color:rgba(239,68,68,0.5); }
        .img-thumb-wrap { position:relative; width:100%; height:140px; background:var(--surface2); overflow:hidden; }
        .img-thumb-wrap img { width:100%; height:100%; object-fit:cover; display:block; transition:transform 0.3s; }
        .img-card:hover .img-thumb-wrap img { transform:scale(1.04); }
        .img-erro-overlay { position:absolute; inset:0; background:rgba(239,68,68,0.12); display:flex; flex-direction:column; align-items:center; justify-content:center; gap:6px; color:#f87171; }
        .check-overlay { position:absolute; top:8px; left:8px; z-index:10; }
        .check-overlay input[type="checkbox"] { width:18px; height:18px; cursor:pointer; accent-color:var(--accent); }
        .status-badge { position:absolute; top:8px; right:8px; z-index:10; font-size:0.6rem; font-weight:800; letter-spacing:0.4px; padding:0.2rem 0.5rem; border-radius:100px; }
        .img-card-info { padding:0.65rem 0.75rem; }
        .img-card-url { font-size:0.68rem; color:var(--text-muted); white-space:nowrap; overflow:hidden; text-overflow:ellipsis; margin-bottom:0.3rem; }
        .img-card-prods { font-size:0.72rem; color:var(--text-sub); white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
        .img-card-actions { display:flex; gap:0.4rem; padding:0 0.75rem 0.75rem; }
        .img-card-actions .btn { flex:1; font-size:0.72rem; padding:0.35rem 0.5rem; justify-content:center; }
        .filtro-bar { display:flex; flex-wrap:wrap; gap:0.5rem; align-items:center; }
        .btn-filtro-tab { padding:0.4rem 1rem; border-radius:100px; font-size:0.78rem; font-weight:700; border:1px solid var(--border); background:var(--surface2); color:var(--text-sub); cursor:pointer; transition:all 0.15s; }
        .btn-filtro-tab.active,.btn-filtro-tab:hover { background:var(--accent); border-color:var(--accent); color:#fff; }
        #toolbarLote { display:none; animation:fadeIn 0.2s; }
        @keyframes fadeIn { from{opacity:0;transform:translateY(-6px)} to{opacity:1;transform:none} }
        #modalEditor .modal-dialog { max-width:680px; }
        #cropContainer { max-height:380px; overflow:hidden; border-radius:10px; background:#000; }
        #cropContainer img { max-width:100%; display:block; }
        .preset-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:6px; }
        .btn-preset { padding:0.4rem; font-size:0.72rem; font-weight:700; background:var(--surface2); border:1px solid var(--border); border-radius:8px; color:var(--text-sub); cursor:pointer; transition:all 0.15s; text-align:center; }
        .btn-preset:hover,.btn-preset.active { border-color:var(--accent); color:var(--accent2); background:rgba(59,130,246,0.1); }
        .empty-state { text-align:center; padding:4rem 1rem; color:var(--text-muted); }
        .empty-state i { font-size:2.5rem; margin-bottom:1rem; opacity:0.35; display:block; }
        .tag-local { background:rgba(16,185,129,0.15); color:#34d399; border:1px solid rgba(16,185,129,0.25); font-size:0.58rem; font-weight:800; padding:0.15rem 0.45rem; border-radius:100px; }
        .tag-link { background:rgba(59,130,246,0.12); color:#93c5fd; border:1px solid rgba(59,130,246,0.2); font-size:0.58rem; font-weight:800; padding:0.15rem 0.45rem; border-radius:100px; }
    </style>
</head>
<body>
<div class="wrapper">
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <div class="nav flex-column nav-pills mt-3 flex-grow-1">
            ...
            <a class="nav-link" href="../geral/" target="_blank"><i class="fa-solid fa-arrow-up-right-from-square"></i> Ver Vitrine</a>
            <div class="sidebar-section-label">Conta</div>
            <a class="nav-link" href="index.php?page=configuracoes"><i class="fa-solid fa-gear"></i> Configurações</a>
        </div>
        ...
        <main class="container-fluid p-3 p-md-4">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
                ...
                <div class="d-flex gap-2">
                    ...
                    <a href="index.php?page=produtos" class="btn btn-outline-light"><i class="fa-solid fa-arrow-left"></i> Voltar</a>
                </div>
            </div>

            <div class="card p-3 mb-4">
                <div class="filtro-bar mb-3">
                    <button class="btn-filtro-tab active" onclick="filtrar('todas',this)"><i class="fa-solid fa-border-all me-1"></i> Todas <span class="ms-1 opacity-75">(<?= $totalImagens ?>)</span></button>
                    <button class="btn-filtro-tab" onclick="filtrar('validas',this)"><i class="fa-solid fa-circle-check me-1"></i> Válidas</button>
                    <button class="btn-filtro-tab" onclick="filtrar('quebradas',this)"><i class="fa-solid fa-circle-xmark me-1"></i> Quebradas</button>
                    <button class="btn-filtro-tab" onclick="filtrar('local',this)"><i class="fa-solid fa-hard-drive me-1"></i> Arquivos locais</button>
                    <button class="btn-filtro-tab" onclick="filtrar('link',this)"><i class="fa-solid fa-link me-1"></i> Links externos</button>
                </div>
                <div class="d-flex align-items-center gap-3 flex-wrap">
                    ...
                    <div id="toolbarLote">
                        ...
                        <button class="btn btn-sm btn-outline-danger" onclick="excluirLote()"><i class="fa-solid fa-trash"></i> Excluir selecionadas</button>
                    </div>
                </div>
            </div>
            ...
            <?php else: ?>
                <form id="formLote" method="POST" action="galeria.php">
                    ...
                    <div class="galeria-admin-grid" id="galeriaGrid">
                        ...
                        ?>
                            <div class="img-card" data-url="<?= $urlEnc ?>" data-pos="<?= $posEnc ?>" data-tipo="<?= $isLocal ? 'local':'link' ?>" data-status="carregando">
                                <div class="check-overlay">
                                    <input type="checkbox" name="urls[]" value="<?= $urlEnc ?>" onchange="atualizarSelecao()">
                                </div>
                                <span class="status-badge <?= $isLocal ? 'tag-local':'tag-link' ?>"><?= $isLocal ? 'LOCAL':'LINK' ?></span>
                                <div class="img-thumb-wrap">
                                    <img src="<?= $urlEnc ?>" alt="" style="object-position:<?= $posEnc ?>;" loading="lazy" onload="marcarValida(this)" onerror="marcarQuebrada(this)">
                                ...
                                <div class="img-card-info">
                                    <div class="img-card-url" title="<?= $urlEnc ?>"><?= basename($url) ?></div>
                                    <div class="img-card-prods" title="<?= htmlspecialchars($nomes ?? '') ?>">
                                        ...
                                        <?php if ($nomes): ?> · <span style="opacity:0.7;"><?= htmlspecialchars(mb_strimwidth($nomes, 0, 40, '…')) ?></span><?php endif; ?>
                                    </div>
                                </div>
                                <div class="img-card-actions">
                                    <button type="button" class="btn btn-outline-light" onclick="abrirEditor('<?= $urlEnc ?>','<?= $posEnc ?>')" title="Editar">
                                        <i class="fa-solid fa-crop-simple"></i> Editar
                                    </button>
                                    <button type="button" class="btn btn-outline-danger" onclick="confirmarExclusao('<?= $urlEnc ?>',<?= $prods ?>)">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header border-0 pb-1">
                <h5 class="modal-title fw-bold"><i class="fa-solid fa-sliders text-primary me-2"></i>Editar Imagem</h5>
            ...
            <ul class="nav nav-pills px-4 pt-2 pb-0 gap-2" id="editorTabs">
                <li class="nav-item"><button class="nav-link active px-3 py-1" onclick="mudarAba('pos',this)"><i class="fa-solid fa-crosshairs me-1"></i> Posição</button></li>
                <li class="nav-item"><button class="nav-link px-3 py-1" onclick="mudarAba('crop',this)"><i class="fa-solid fa-crop-simple me-1"></i> Recortar</button></li>
            </ul>
            <div class="modal-body px-4 pt-3 pb-2">
                <div id="abaPos">
                    <p class="small text-muted mb-2">Clique na imagem para definir o ponto de foco usado no card da vitrine.</p>
                    <div id="posThumb" style="position:relative;width:100%;height:220px;border-radius:10px;overflow:hidden;border:1px solid var(--border);cursor:crosshair;background:var(--surface2);" onclick="clicarFoco(event,this)">
                        <img id="posImg" src="" alt="" style="width:100%;height:100%;object-fit:cover;display:block;">
                        <div id="miraFoco" style="display:none;position:absolute;width:20px;height:20px;border-radius:50%;background:rgba(255,255,255,0.9);border:2px solid var(--accent);transform:translate(-50%,-50%);pointer-events:none;box-shadow:0 0 0 4px rgba(59,130,246,0.35),0 2px 8px rgba(0,0,0,0.5);"></div>
                    ...
                    <div class="mt-2 d-flex align-items-center gap-2">
                        ...
                        <input type="text" id="inputFocoManual" class="form-control form-control-sm" placeholder="50% 30%" oninput="aplicarFocoManual(this.value)">
                    </div>
                </div>
                <div id="abaCrop" style="display:none;">
                    <p class="small text-muted mb-2">Recorte a imagem. Será salva e aplicada a todos os produtos que a usam.</p>
                    ...
                    <div id="cropContainer"><img id="cropImgTarget" src="" style="max-width:100%;display:block;"></div>
                    <div class="d-flex gap-2 mt-2 justify-content-center flex-wrap">
                        <button type="button" class="btn btn-sm btn-outline-light" onclick="cropper.rotate(-90)"><i class="fa-solid fa-rotate-left"></i></button>
                        <button type="button" class="btn btn-sm btn-outline-light" onclick="cropper.rotate(90)"><i class="fa-solid fa-rotate-right"></i></button>
                        <button type="button" class="btn btn-sm btn-outline-light" onclick="cropper.zoom(0.1)"><i class="fa-solid fa-magnifying-glass-plus"></i></button>
                        <button type="button" class="btn btn-sm btn-outline-light" onclick="cropper.zoom(-0.1)"><i class="fa-solid fa-magnifying-glass-minus"></i></button>
                        <button type="button" class="btn btn-sm btn-outline-light" onclick="cropper.reset()"><i class="fa-solid fa-arrows-rotate"></i></button>
                        <button type="button" class="btn btn-sm btn-outline-light" onclick="cropper.scaleX(cropper.getData().scaleX*-1)"><i class="fa-solid fa-left-right"></i></button>
                    </div>
                </div>
            </div>
            <div class="modal-footer border-0 pt-0 px-4 pb-4 gap-2">
                <button type="button" class="btn btn-outline-light" onclick="fecharEditor()">Cancelar</button>
                <button type="button" class="btn btn-success" onclick="salvarEditor()"><i class="fa-solid fa-floppy-disk"></i> Salvar</button>
            </div>
        </div>
    </div>
</div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer border-0 pt-0">
                ...
                <form method="POST" action="galeria.php" style="display:inline;">
                    ...
                    <button type="submit" class="btn btn-danger"><i class="fa-solid fa-trash me-1"></i> Excluir</button>
                </form>
            </div>
        </div>
    </div>
...
<script>
let cropper = null, abaAtiva = 'pos', urlEditando = '', focoAtual = 'center center';
let modalEditor = null, modalExcluir = null, totalQuebradas = 0;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    setTimeout(() => {
        if (totalQuebradas === 0) document.getElementById('contQuebradasLabel').textContent = '';
    }, 4000);
});
...
function marcarQuebrada(imgEl) {
    ...
    totalQuebradas++;
    document.getElementById('contQuebradasLabel').textContent = totalQuebradas + ' quebrada(s)';
}
...
function filtrar(tipo, btn) {
    ...
    document.querySelectorAll('.img-card').forEach(card => {
        let show = tipo === 'todas' || (tipo === 'validas' && card.dataset.status === 'valida') || (tipo === 'quebradas' && card.dataset.status === 'quebrada') || (tipo === 'local' && card.dataset.tipo === 'local') || (tipo === 'link' && card.dataset.tipo === 'link');
...
function atualizarSelecao() {
    ...
    document.getElementById('countSelecionados').textContent = checks.length;
    document.getElementById('toolbarLote').style.display = checks.length > 0 ? 'inline-flex' : 'none';
    document.querySelectorAll('.img-card').forEach(card => {
...
function excluirLote() {
    ...
    if (!confirm(`Excluir ${checks.length} imagem(ns) selecionada(s) e desvincular de todos os produtos?`)) return;
...
function confirmarExclusao(url, totalProd) {
    ...
    const aviso = totalProd > 0 ? `<strong class="text-white">${totalProd} produto(s)</strong> ficarão sem imagem.` : 'Esta imagem não está vinculada a nenhum produto.';
    document.getElementById('excluirMsg').innerHTML = `Tem certeza que deseja excluir esta imagem?<br><br>${aviso}<br><br><span style="font-size:0.78rem;opacity:0.6;word-break:break-all;">${url.replace(/</g,'&lt;')}</span>`;
    modalExcluir.show();
}

function abrirEditor(url, pos) {
    urlEditando = url; focoAtual = pos || 'center center';
...
function fecharEditor() { if (cropper) { cropper.destroy(); cropper = null; } modalEditor.hide(); }
...
function aplicarFocoManual(val) { if (val.trim()) aplicarFoco(val.trim(), null, null); }
function aplicarFoco(pos, xPct, yPct) {
    ...
    if (xPct && yPct) {
        mira.style.display = guiaH.style.display = guiaV.style.display = 'block';
        mira.style.left = xPct; mira.style.top = yPct;
        guiaH.style.top = yPct; guiaV.style.left = xPct;
    } else { mira.style.display = guiaH.style.display = guiaV.style.display = 'none'; }
}

function iniciarCropper() {
    if (cropper) { cropper.destroy(); cropper = null; }
    const img = document.getElementById('cropImgTarget');
    if (!img.complete || img.naturalWidth === 0) img.onload = () => criarCropper(img);
    else criarCropper(img);
}
function criarCropper(img) {
    cropper = new Cropper(img, { aspectRatio:1, viewMode:1, autoCropArea:0.9, background:false, movable:true, rotatable:true, scalable:true, zoomable:true });
}
function setAspecto(ratio) { if (cropper) cropper.setAspectRatio(ratio); }

function salvarEditor() {
    const form = document.createElement('form');
    form.method = 'POST'; form.action = 'galeria.php';
    const campos = { action:'salvar_crop', url_original:urlEditando, imagem_pos:focoAtual, imagem_base64:'' };
    if (abaAtiva === 'crop' && cropper) {
        campos.imagem_base64 = cropper.getCroppedCanvas({ width:800, height:800, imageSmoothingQuality:'high' }).toDataURL('image/png');
    }
